@php
    ...
    $linker = json_encode(['from' => $start, 'to' => $end]);
    $link = '?rep=' . urlencode($linker);
@endphp
@extends('layouts.admin.app')
@section('page-name')
    ACTIVITY REPORT FOR AGENCIES | From : {{ $start }} , To : {{ $end }}
@endsection
@section('body')
    @php
            ...
            <div class="card">
                <div class="card-header border-bottom-dashed">
                    <div class="row g-4 align-items-center">
                        <div class="col-sm">
                            <div>
                                <h5 class="card-title mb-0">BRANCH ACTIVITY REPORT FOR AGENCIES | From : {{ $start }} , To :
                                    {{ $end }}</h5>
                            </div>
                        </div>
                            ...
                            @while ($currentDate <= $endDate)
                                <tr>
                                    ...
                                    @php
                                        ...
                                        $sql = "SELECT max(a.name),SUM(IFNULL(b.weight,0)) AS weight, SUM(IFNULL(b.amount,0)) AS amount, SUM(IFNULL(b.postage,0)) AS postage FROM branches a LEFT JOIN `outboxing` b ON(a.id=b.blanch $v) WHERE b.blanch  = $branchId  GROUP BY a.id";
                                        //echo $sql."<br>";
                                        $res = DB::select($sql);
                                        ...
                                        unset($res);
                                        unset($row);
                                        $sql = "SELECT max(a.name),SUM(IFNULL(b.weight,0)) AS weight, SUM(IFNULL(b.amount,0)) AS amount, SUM(IFNULL(b.postage,0)) AS postage FROM branches a LEFT JOIN `poutboxing` b ON(a.id=b.blanch $v) WHERE b.blanch  = $branchId GROUP BY a.id";
                                        //echo $sql."<br>";
                                        $res = DB::select($sql);
                                        ...
                                        unset($res);
                                        unset($row);
                                        $sql = "SELECT max(a.name),SUM(IFNULL(b.weight,0)) AS weight, SUM(IFNULL(b.amount,0)) AS amount, SUM(IFNULL(b.postage,0)) AS postage FROM branches a LEFT JOIN `registoutboxing` b ON(a.id=b.blanch $v) WHERE b.blanch  = $branchId GROUP BY a.id";
                                        //echo $sql."<br>";
                                        $res = DB::select($sql);
                                        ...
                                        unset($res);
                                        unset($row);
                                        $sql = "SELECT max(a.name),SUM(IFNULL(b.weight,0)) AS weight, SUM(IFNULL(b.temb_amount,0)) AS amount, SUM(IFNULL(b.temb_amount,0)) AS postage FROM branches a LEFT JOIN `tembleoutboxing` b ON(a.id=b.blanch $v) GROUP BY a.id";
                                        //echo $sql."<br>";
                                        $res = DB::select($sql);
                                        ...
                                        $sql = "SELECT MAX(a.name) AS name,
               SUM(IFNULL(b.weight, 0)) AS weight,
               SUM(IFNULL(b.amount, 0)) AS amount,
               SUM(IFNULL(b.postage, 0)) AS postage
        FROM branches a
        LEFT JOIN `outboxing` b ON (a.id = b.blanch $v)
        WHERE b.blanch  = $branchId";
                                        $res = DB::select($sql);
...
WHERE b.blanch  = $branchId";
                                    ...
                                    <td>
                                        <a type="button" class="btn btn-primary btn-sm edit-item"
                                            href="{{ route('branch.breporting.index', $link) }}"><span>View
                                                Detailed</span></a>
                                    </td>
                                </tr>
                                @php
