...
<body>
    <div class="watermark">PP VIRTUAL P.O BOX INCOME {{ decrypt($pdate) }}</div>
 <table width="100%">
<tr>
<td  width="25%">
...
<h1 style="text-align:center">PP VIRTUAL P.O BOX  INCOME REPORT   </h1>
...
<table width="100%">
 ...
 <tr>
 <td>{{ $key + 1 }}</td>
 <td>{{$inboxing->box->phone }}</td>
<td>{{ $inboxing->year }}</td>
<td>{{ $inboxing->payment_type }}</td>
<td>{{ $inboxing->payment_model }}</td>
<td>{{ number_format($inboxing->amount) }} </td>
@php
$total += $inboxing->amount;
@endphp
<tr>
 @endforeach
<tr>
