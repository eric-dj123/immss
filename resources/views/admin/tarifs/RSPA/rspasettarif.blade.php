                ...
                <div class="row g-4 align-items-center">
                    <div class="col-sm">
                        @foreach ($zones as $key =>$zone)
                                <div>
                                    <h5 class="card-title mb-0">TARRIF REGISTRATION IN {{ $zone->Continent }}</h5>
                                </div>
                                @endforeach
                            </div>
                            ...
                            <tbody>
                                @foreach ($sets as $key =>$set)
                                    <tr>
                                        ...
                                        <td class="name"><a href="">{{ $set->minweight }} - {{ $set->maxweight }}</a>
                                        </td>
                                        <td class="email">{{ $set->amount }}</td>
                                        </td>
                                        <td>
                                            <a href="#showModals{{ $set->pcon_id }}" data-bs-toggle="modal"
                                                type="button" class="btn btn-primary btn-sm"><span>Edit</span></a>
                                            <div class="modal fade" id="showModals{{ $set->pcon_id }}" tabindex="-1"
                                                aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                        ...
                                                        <form class="tablelist-form" id="myForm" method="post"
                                                            action="{{ route('admin.rspatreg.rspatregup', $set->pcon_id) }}">
                                                            @csrf
                                                            @method('PUT')
    ...
    <div class="modal fade" id="showModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form class="tablelist-form" method="post" action="{{ route('admin.rspatreg.storetarifp') }}">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                            @if ($errors->any())
                        ...
                        <input type="hidden" name="servty_id" id=""value="{{ $servty_id }}">
                        <input type="hidden" name="cont_id" id=""value="{{ $cont_id }}">
                        <input type="hidden" name="type" id=""value="g">

                        <div class="mb-3">
                                <label for="branch" class="form-label">Select Range/Gramme</label>
                                <select class="form-control" name="range"
                                    id="branch" required>
                                    <option value="" disabled selected>Select Range</option>
                                    @foreach ($results as $result)
                                    <option @if (old('range')==$result->id) selected @endif
                                        value="{{ $result->minweight }},{{ $result->maxweight }}">{{$result->minweight }}-{{$result->maxweight }}</option>
                                    @endforeach
