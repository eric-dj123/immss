...
class GoogleadIncomeMailController extends Controller
{
    public function index()
    {
        ...
        ->limit(30)
        ->get();
        return view('admin.backoffice.googleincomes', compact('courierPays'));
    }
...
if ($existingIncome->count() >= 1) {
    ...
    public function transactiongoogle($pdate)
    {
        ...
        ->where('courierpays.extra','GAD')
        ->where('courierpays.userid',auth()->user()->id)
        ->get();

        $pdf = Pdf::loadView('admin.backoffice.googleinvoice', compact('pdate','inboxings'))
        ->setPaper('a4', 'portrait');
        return $pdf->stream('transactiongoogle.pdf');
    }
}
