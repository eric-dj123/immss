...
class registoutboxing extends Model
{
    use HasFactory;
    protected $table = 'registoutboxing';
    ...
    public static function byrange($start, $end)
    {
        ...
        if (isset($start) && !empty($start) && isset($end) && !empty($end)) {
            if ($start != $end) {
                $v = "AND registoutboxing.created_at BETWEEN '$start' AND '$end'";
            } else {
                $regdate = $start;
                $v = "AND DATE(registoutboxing.created_at)='$regdate'";
            }
        } else {
            $regdate = date('Y-m-d');
            $v = "AND DATE(registoutboxing.created_at)='$regdate'";
        ...
        $sql = "SELECT * FROM `registoutboxing` WHERE blanch='$branch' $v";
    ...
    {
        return DB::table('registoutboxing')
            ->select(
    ...
    public static function ems_report($start = '', $end = '')
    {
        ...
        $query = DB::table('branches as a')
            ->leftJoin('registoutboxing AS b', function ($join) use ($v) {
