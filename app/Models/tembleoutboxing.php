...
class tembleoutboxing extends Model
{
    use HasFactory;
    protected $table = 'tembleoutboxing';
    ...
    public static function byrange($start, $end)
    {
        ...
        if (isset($start) && !empty($start) && isset($end) && !empty($end)) {
            if ($start != $end) {
                $v = "AND tembleoutboxing.created_at BETWEEN '$start' AND '$end'";
            } else {
                $regdate = $start;
                $v = "AND DATE(tembleoutboxing.created_at)='$regdate'";
            }
        } else {
            $regdate = date('Y-m-d');
            $v = "AND DATE(tembleoutboxing.created_at)='$regdate'";
        ...
        $sql = "SELECT * FROM `tembleoutboxing` WHERE blanch='$branch' $v";
    ...
    {
        return DB::table('tembleoutboxing')
            ->select(
                DB::raw('SUM(amount) as amount'),
                DB::raw('SUM(temb_amount) as temb_amount'),
    ...
    public static function ems_report($start = '', $end = '')
    {
        ...
        $query = DB::table('branches as a')
            ->leftJoin('tembleoutboxing AS b', function ($join) use ($v) {
            ...
            ->select(
                ...
                DB::raw('SUM(IFNULL(b.temb_amount, 0)) AS temb_amount'),
