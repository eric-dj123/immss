...
class Outboxing extends Model
{
    use HasFactory;
    protected $table = 'outboxing';
    ...
    public static function byrange($start, $end)
    {
        ...
        if (isset($start) && !empty($start) && isset($end) && !empty($end)) {
            if ($start != $end) {
                $v = "AND DATE(outboxing.created_at) BETWEEN '$start' AND '$end'";
            } else {
                $regdate = $start;
                $v = "AND DATE(outboxing.created_at)='$regdate'";
            }
        } else {
            $regdate = date('Y-m-d');
            $v = "AND DATE(outboxing.created_at)='$regdate'";
        ...
        $sql = "SELECT * FROM `outboxing` WHERE blanch='$branch' $v";
    ...
    public static function ems_report($start = '', $end = '')
    {
        ...
        $query = DB::table('branches as a')
            ->leftJoin('outboxing AS b', function ($join) use ($v) {
